...
class VectorSearchClient:
    ...
    def create_endpoint(
        ...
        :param int target_qps: Target queries per second for the endpoint. Must be a positive integer.
                               Capacity is automatically adjusted at index creation/sync time to best
                               match this throughput target (best-effort, not guaranteed). Optional.

        .. note::
           **BETA FEATURE**: The target_qps parameter is in beta and subject to change.
           It enables automatic capacity scaling based on desired throughput.
           Currently only supported for STANDARD endpoints.

        .. warning::
           Only STANDARD endpoints support target_qps. Storage Optimized endpoints will
           return an error if target_qps is specified.
        ...
        **Example**::
        ...
        if target_qps is not None:
            if not isinstance(target_qps, int):
                raise InvalidInputException("target_qps must be an integer.")
            if target_qps <= 0:
                raise InvalidInputException("target_qps must be a positive integer.")
        ...
        if target_qps is not None:
            json_data["target_qps"] = target_qps
    ...
    def update_endpoint(
        ...
        :param int target_qps: Target queries per second for the endpoint. Must be a positive
                               integer. Optional.
        ...
        **Example**::
        ...
        if target_qps is not None:
            if not isinstance(target_qps, int):
                raise InvalidInputException("target_qps must be an integer.")
            if target_qps == 0 or target_qps < -1:
                raise InvalidInputException("target_qps must be a positive integer.")
    ...
    def create_delta_sync_index(
        ...
        :param str index_subtype: The subtype of the index. "HYBRID" (default) supports semantic search
                                  with embeddings and keyword search. "FULL_TEXT" supports keyword search
                                  only, without embeddings.

        .. note::
           **BETA FEATURE**: The index_subtype parameter is in beta and subject to change.
           "FULL_TEXT" enables keyword-only search without embeddings.
           Currently only supported for STORAGE_OPTIMIZED endpoints.

        .. warning::
           Only STORAGE_OPTIMIZED endpoints support index_subtype="FULL_TEXT". STANDARD endpoints
           will return an error if FULL_TEXT is specified.
        ...
        is_full_text = index_subtype and index_subtype.upper() == "FULL_TEXT"
        json_data = {
            "name": index_name,
        ...
        if index_subtype:
            json_data["index_subtype"] = index_subtype.upper()
        ...
        elif not is_full_text:
            raise InvalidInputException(
                "At least one valid embedding_source_column or embedding_vector_column must be specified."
            )
    ...
    def get_index(self, endpoint_name=None, index_name=None):
        ...
        """
        params = self._fetch_index_params(endpoint_name, index_name)
        return VectorSearchIndex(
            workspace_url=self.workspace_url,
            index_url=params["index_url"],
            get_reranker_url_callable=params["get_reranker_url"],
            personal_access_token=self.personal_access_token,
            service_principal_client_id=self.service_principal_client_id,
            service_principal_client_secret=self.service_principal_client_secret,
            name=index_name,
            endpoint_name=params["response_endpoint_name"],
            mlserving_endpoint_name=params["mlserving_endpoint_name"],
            azure_tenant_id=self.azure_tenant_id,
            azure_login_id=self.azure_login_id,
            use_user_passed_credentials=self._using_user_passed_credentials,
            credential_strategy=self.credential_strategy,
            mlserving_endpoint_name_for_query=params["mlserving_endpoint_name_for_query"],
        )

    def get_async_index(self, endpoint_name=None, index_name=None):
        """
        Get an async index for use in ``async/await`` code.

        Returns an :class:`~databricks.vector_search.async_index.AsyncVectorSearchIndex`
        that mirrors the data-plane methods of :class:`VectorSearchIndex`
        (``upsert``, ``delete``, ``similarity_search``, ``scan``, ``describe``,
        ``sync``) over ``httpx.AsyncClient``. Control-plane index lifecycle
        (create / delete / ``wait_until_ready``) remains on the sync client.

        :param Option[str] endpoint_name: The optional name of the endpoint.
        :param str index_name: The name of the index.
        :returns: An async index bound to the event loop where its first
            method is awaited. Sharing across loops is unsupported.
        :rtype: :class:`databricks.vector_search.async_index.AsyncVectorSearchIndex`
        :raises NotFound: If the index does not exist.
        :raises PermissionDenied: If user lacks permission to view the index.

        .. note::
            This method issues a synchronous HTTP request to resolve the index URL.
            In async contexts (e.g. FastAPI handlers), wrap the call in
            ``asyncio.to_thread``::
    ...
    def _fetch_index_params(self, endpoint_name, index_name):
        """
        Issue the GET index HTTP request and parse the fields needed to construct
        a sync or async index object.

        :param endpoint_name: The optional name of the endpoint.
        :param index_name: The name of the index (must be non-empty).
        :returns: A dict with keys ``index_url``, ``response_endpoint_name``,
            ``mlserving_endpoint_name``, ``mlserving_endpoint_name_for_query``,
            and ``get_reranker_url`` (a zero-argument callable).
        """
        assert index_name, "Index name must be specified"
        resp = RequestUtils.issue_request(
            url=self._get_index_url(endpoint_name, index_name),
        ...
        return {
            "index_url": index_url,
            "response_endpoint_name": response_endpoint_name,
            "mlserving_endpoint_name": mlserving_endpoint_name,
            "mlserving_endpoint_name_for_query": mlserving_endpoint_name_for_query,
            "get_reranker_url": get_reranker_url,
        }
