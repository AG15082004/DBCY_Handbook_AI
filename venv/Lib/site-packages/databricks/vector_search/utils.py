...
def _camel_to_snake(name):
    """Convert a single camelCase string to snake_case. No-op on snake_case input."""
    return _CAMEL_TO_SNAKE_RE.sub("_", name).lower()


def normalize_response_keys_to_snake_case(value):
    """
    Recursively convert camelCase dict keys to snake_case.

    The proto3 JSON canonical mapping spec emits camelCase by default, which is what
    `vector-store` (Rust, via `protoc-gen-prost-serde`) returns on direct-DP requests.
    Responses routed through `brickindex-scheduler` (Scala, via `scalapb-json4s`) come
    back in snake_case because that library defaults the other way. This helper
    normalizes either shape to snake_case so downstream code can use a single key form.

    Already-snake_case keys are passed through unchanged. Lists and scalars are walked
    recursively. None is preserved.
    """
    if isinstance(value, dict):
        return {
            _camel_to_snake(k) if isinstance(k, str) else k: normalize_response_keys_to_snake_case(v)
            for k, v in value.items()
        }
    if isinstance(value, list):
        return [normalize_response_keys_to_snake_case(item) for item in value]
    return value
...
class RequestUtils:
    session = _cached_get_request_session(total_retries=3, backoff_factor=1, backoff_jitter=0.2, process_id=os.getpid())
...
def is_in_model_serving_environment():
    ...
    is_in_model_serving_env = (
        os.environ.get("IS_IN_DB_MODEL_SERVING_ENV") or os.environ.get("IS_IN_DATABRICKS_MODEL_SERVING_ENV") or "false"
    )
